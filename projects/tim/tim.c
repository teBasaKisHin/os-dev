...
err:
				api_putstr0(" >tim file [-w30 -h10 -t4]\n");
				api_end();
			}
		} else {	/* ?t?@?C???????? */
			if (q != 0) {
	...
	/* ?E?B???h?E????? */
	win = api_openwin(winbuf, w * 8 + 16, h * 16 + 37, -1, "tim");
	...
	*r = 0;
	...
	txtbuf[0] = 0x0a; /* ????p????s?R?[?h */
	api_fread(txtbuf + 1, j, i);
	api_fclose(i);
	txtbuf[j + 1] = 0;
	q = txtbuf + 1;
	for (p = txtbuf + 1; *p != 0; p++) {	/* ????????P????????0x0d??R?[?h?????? */
		if (*p != 0x0d) {
			*q = *p;
			q++;
		}
